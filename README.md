...
KOReader 主菜单入口为“Kindle Agent 看板 → 打开 V13 看板”。月历区域滑动切月份，点月份标题回到当月；日程区域分别滑动翻列表。点击条目查看完整内容，左下角太阳调前光，右上角齿轮提供同步状态/常显/清除残影/退出。Wi‑Fi 和电池图标只显示状态。

点击右侧月历日期后，右下方显示该日程；点击具体日程即可根据日程日期和标题设置本地倒计时目标。倒计时显示在左侧日程下方，不依赖 Hub。目标日期支持多个，当前显示主目标和最多两个辅助目标。
