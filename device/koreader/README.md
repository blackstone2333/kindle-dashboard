...
- 月历数字区域上/左滑到下月，下/右滑到上月；点月份标题回到当月。目前可查看已同步的上月、当月、下月。
- 点击月历中的日期，右下方显示该日期的日程和待办；选中日期会显示边框，点击另一个日期即可切换。再次点击当前选中的日期，或点击右下方日期标题，可恢复“下一周的日程”。
- 点击月历日期后会打开操作菜单，可直接将该日期设为主倒计时目标，或新增一个命名目标；倒计时保存在 Kindle 本地，不依赖 Hub。
...
倒计时配置保存在 `settings/kindle-agent-dashboard/countdown.json`。日期由月历点击产生，目标日按 Kindle 的本地日历日计算；当前页面显示一个主目标和最多两个辅助目标。
